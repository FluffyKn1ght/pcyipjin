...
u8 mem_read(Memory* mem, MemoryAccess access, unsigned _BitInt(21) addr) {
    for (int i = 0; i < mem->device_count; i++) {
        if (mem->devices[i].start_addr <= addr && mem->devices[i].end_addr >= addr) {
            return mem->devices[i].read(mem->devices[i].userdata, access, addr);
        }
    }
...
void mem_write(Memory* mem, MemoryAccess access, unsigned _BitInt(21) addr, u8 value) {
    for (int i = 0; i < mem->device_count; i++) {
        if (mem->devices[i].start_addr <= addr && mem->devices[i].end_addr >= addr) {
            mem->devices[i].write(mem->devices[i].userdata, access, addr, value);
            return;
        }
    }
}
